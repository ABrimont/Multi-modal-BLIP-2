...
@registry.register_model("blip2_t5_separate")
class Blip2T5_separate(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    def __init__(
        ...
        self.Qformer, self.query_tokens = self.init_Qformer(num_query_token, 1408, cross_attention_freq=cross_attention_freq)
        self.Qformer_aud, self.query_tokens_aud = self.init_Qformer(num_aud_query_token, 768, cross_attention_freq=cross_attention_freq)
        ...
        self.Qformer_aud.cls = None
        self.Qformer_aud.bert.embeddings.word_embeddings = None
        self.Qformer_aud.bert.embeddings.position_embeddings = None
        for layer in self.Qformer_aud.bert.encoder.layer:
            layer.output = None
            layer.intermediate = None
    ...
    def forward(self, samples):
        if not self.scst: # xe training
            ...
            query_output_aud = self.Qformer_aud.bert(
            query_embeds=self.query_tokens_aud,
            encoder_hidden_states= audio_features,
            encoder_attention_mask = audio_mask_LLM,
            return_dict=True,
        )
            
            query_output_vis = self.Qformer.bert(
